...
#define ARBINT_SHA256_BSIG0(x)                                                \
  (arbint_sha256_rotr((x), 2) ^ arbint_sha256_rotr((x), 13) ^                 \
   arbint_sha256_rotr((x), 22))
#define ARBINT_SHA256_BSIG1(x)                                                \
  (arbint_sha256_rotr((x), 6) ^ arbint_sha256_rotr((x), 11) ^                 \
   arbint_sha256_rotr((x), 25))
...
static const uint32_t crc_table[4][256] = {
...
uint32_t arbint_crc32c_generic(uint32_t crc, const uint8_t * buf, size_t len) {
  for (; len && ((uintptr_t) buf & 3); --len)
    crc = (crc >> 8) ^ crc_table[0][(crc & 0xFF) ^ *buf++];
  for (; len >= 4; buf += 4, len -= 4) {
    crc ^= *(const uint32_t *) buf;
    crc = crc_table[0][crc >> 24] ^ crc_table[1][(crc >> 16) & 0xFF] ^
          crc_table[3][crc & 0xFF] ^ crc_table[2][(crc >> 8) & 0xFF];
  }
  for (; len; --len)
    crc = (crc >> 8) ^ crc_table[0][(crc & 0xFF) ^ *buf++];
  return crc;
