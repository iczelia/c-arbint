dnl ARBINT_CHECK_X86_FEATURE_WITH_FLAG
dnl ---------------------------------
dnl Probe an x86 feature by compiling and linking a test program first without
...
dnl  4: compiler flag to retry with (e.g. -mbmi2)
...
AC_DEFUN([ARBINT_CHECK_X86_FEATURE_WITH_FLAG], [
  AC_CACHE_CHECK([for x86 $1 without extra flags], [$2], [
    AS_IF([test "x$arbint_host_x86" != "xyes"], [
      AS_VAR_SET([$2], [no])
    ], [
  ...
  AS_VAR_IF([$2], [yes], [], [
    AC_CACHE_CHECK([for x86 $1 with $4], [$3], [
      AS_IF([test "x$arbint_host_x86" != "xyes"], [
        AS_VAR_SET([$3], [no])
      ], [
